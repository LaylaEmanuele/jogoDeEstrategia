...
public class Principal extends JFrame {
	...
	public JogadorAux jogador2;
	public List<String> ataques = new ArrayList<>();
	...
	public Principal() {
		...
		// "Jogador", JOptionPane.QUESTION_MESSAGE);
		// String[] civilizacoes = { "Acádia", "Babilônia", "Helenística",
		// "Mesopotâmica", "Persa", "Suméria" };
		// String civilizacao = (String) JOptionPane.showInputDialog(null, "Escolha sua
		// civilização", "Jogador",
		// JOptionPane.QUESTION_MESSAGE, null, civilizacoes, civilizacoes[0]);
		// this.lblJogador.setText(nome + " - " + civilizacao);
		// Civilizacao civilizacaoJ = new Civilizacao(civilizacao, this);
		// this.jogador = new Jogador(nome, 1, civilizacaoJ);
	...
	private void initialize() {
		...
		this.cbCivilizacoes.addItem("Ac�dia");
		this.cbCivilizacoes.addItem("Babil�nia");
		this.cbCivilizacoes.addItem("Helen�stica");
		this.cbCivilizacoes.addItem("Mesopot�mica");
		this.cbCivilizacoes.addItem("Persa");
		this.cbCivilizacoes.addItem("Sum�ria");
		this.pnCivilizacao.add(this.cbCivilizacoes);

		String[] colunasJogadores = { "Jogador", "Civiliza��o", "IP", "Situa��o" };
		...
		this.pnMensagens.setBorder(
				new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)),
						"Mensagens", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		...
		pnFazenda.setBorder(
				new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)),
						"Fazendas", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		...
		this.pnTemplo.setBorder(
				new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)),
						"Templo", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		...
		this.pnOferenda.setBorder(
				new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)),
						"Oferendas de f�", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		...
		this.cbTEmploEvolucoes.addItem("Morte dos primogenitos");
		this.cbTEmploEvolucoes.addItem("Chuva de pedras");
		this.cbTEmploEvolucoes.addItem("Protecao contra nuvem de gafanhotos");
		this.cbTEmploEvolucoes.addItem("Protecao contra morte dos primognitos");
		this.cbTEmploEvolucoes.addItem("Protecao contra chuva de pedras");
		...
		this.pnMaravilha.setBorder(
				new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)),
						"Maravilha", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		...
		this.tfMensagem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				System.out.println("ADICIONAR MSG 2: " + tfMensagem.getText());
		...
		this.btnEnviar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				System.out.println("ADICIONAR MSG 1: " + tfMensagem.getText());
				comandoEnviarMensagem(tfMensagem.getText(), cliente);
			}
		});
		...
		btnTemploLancar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				comandoTemploLancar(cbTemploLancamentos.getSelectedItem().toString(),
						(String)cbTemploInimigo.getSelectedItem());
			}
		});
	...
	public void mostrarAtaques(List<String> evolucoes) {
		...
		for (String evolucao : evolucoes) {
			cbTemploLancamentos.addItem(evolucao);
		}
	}
	...
	private void comandoCriarJogo(String nome, String civilizacao) {
		...
		if (retorno) {
			...
			JogadorAux jogador = new JogadorAux(nome, 1, civilizacao, ipLocal);
			this.situacaoInicio = SituacaoInicio.CRIAR_CRIADO;
			
			civilizacaoJ = new Civilizacao(civilizacao, this);
			this.habilitarInicio();
			Jogador j= new Jogador(nome, 1, civilizacaoJ);
			j.getCivilizacao().getVila().getTemplo().setConstruido(true);
			...
			if (cliente.clienteTCP.conectar(ipLocal, nome, jogador2, this)) {
...
//			System.out.println("------JOGADOR INICIAR JOGO"+jogador);
			cliente.clienteTCP.liberarTela();
...
//			this.iniciar();
	...
	private void comandoConectar(String nome, String civilizacao, String ipServidor) {
		...
		if (retorno && ipConectado == true) {
			try {
				String ipLocal = InetAddress.getByName("localhost").getHostAddress();
				JogadorAux jogador = new JogadorAux(nome, 1, civilizacao, ipLocal);
				...
				System.out.println("------JOGADOR CONECTAR"+j);
				this.habilitarMaravilha();

				// se conectar ao cliente
				cliente = new Cliente(this);

				if (cliente.clienteTCP.conectar(ipServidor, nome, jogador2, this)) {
		...
		this.tpJogo.setSelectedIndex(0); // começa na tela inicial
        this.tpJogo.setEnabledAt(1, false); // 
	...
	public void comandoAldeaoParar(int aldeao) {
		if (aldeao == -1)
			...
			Utils.getAldeaoById(this.jogador.getCivilizacao().getVila().getAldeoes(), aldeao + 1).setAcao(AcaoAldeao.NADA);
	}

	public void comandoAldeaoConstruir(int aldeao, String construcao) {
		String[] info = { "", construcao };
		if (aldeao == -1)
			mostrarMensagemErro("Erro", "Escolha um aldeão");
		else
			Utils.getAldeaoById(this.jogador.getCivilizacao().getVila().getAldeoes(), aldeao + 1)
			.setAcao(AcaoAldeao.CONSTRUIR, info);
	}

	public void comandoAldeaoCultivar(int aldeao, int numeroFazenda) {
		String[] info = { String.valueOf(numeroFazenda + 1), "" };
		if (aldeao == -1)
			mostrarMensagemErro("Erro", "Escolha um aldeão");
		else
			Utils.getAldeaoById(this.jogador.getCivilizacao().getVila().getAldeoes(), aldeao + 1).setAcao(AcaoAldeao.CULTIVAR,
					info);
	}

	public void comandoAldeaoMinerar(int aldeao, int numeroMinaOuro) {
		String[] info = { String.valueOf(numeroMinaOuro + 1), "" };
		if (aldeao == -1)
			mostrarMensagemErro("Erro", "Escolha um aldeão");
		else
			Utils.getAldeaoById(this.jogador.getCivilizacao().getVila().getAldeoes(), aldeao + 1).setAcao(AcaoAldeao.MINERAR,
					info);
	}
	...
	public void comandoAldeaoOrar(int aldeao) {
		if (aldeao == -1)
			...
			Utils.getAldeaoById(this.jogador.getCivilizacao().getVila().getAldeoes(), aldeao + 1).setAcao(AcaoAldeao.ORAR);
	}
	...
	public void comandoAldeaoSacrificar(int aldeao) {
		if (aldeao == -1)
			...
			Utils.getAldeaoById(this.jogador.getCivilizacao().getVila().getAldeoes(), aldeao + 1)
			.setAcao(AcaoAldeao.SACRIFICAR);
	}

	public void comandoPrefeituraCriarAldeao() {
		this.jogador.getCivilizacao().getVila().getPrefeitura().setAcao("Criando aldeão");
	}

	public void comandoPrefeituraEvoluir(String strEvolucao) {
		this.jogador.getCivilizacao().getVila().getPrefeitura().setAcao(strEvolucao);
	}
	...
	public void comandoTemploEvoluir(String strEvolucao) {
		switch (strEvolucao) {
			...
			case "Chuva de pedras":
				if(!ataques.contains(strEvolucao)) {
					this.cbTEmploEvolucoes.removeItem(strEvolucao);
					this.jogador.getCivilizacao().getVila().getTemplo().setAcao(strEvolucao);
					ataques.add(strEvolucao);
					mostrarAtaques(ataques);					
				}
				break;
			case "Protecao contra nuvem de gafanhotos": 
			case "Protecao contra morte dos primognitos":
			case "Protecao contra chuva de pedras":
				if(!this.jogador2.getProtecoes().contains(strEvolucao)) {
					this.cbTEmploEvolucoes.removeItem(strEvolucao);
					this.jogador.getCivilizacao().getVila().getTemplo().setAcao(strEvolucao);
					jogador2.adicionarProtecao(strEvolucao);
					cliente.clienteTCP.atualizarJogador(jogador2);
				}
				break;
			default:
				break;
	...
	public void comandoTemploLancar(String praga, String inimigo) {
		System.out.println("comandoTemploLancar(" + praga + "," + inimigo + ");");
		
		cliente.clienteTCP.lancarAtaque(praga, inimigo, this.jogador2);
	}
